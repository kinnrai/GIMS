    ...
    <el-dialog title="提示" :visible.sync="dialogVisible" width="600px" :before-close="handleClose">
      <el-form :model="editForm" :rules="rules" ref="editForm" label-width="100px" employ="demo-ruleForm">
        ...
        <el-form-item label="公司" prop="companyId" label-width="100px">
          <el-select v-model="editForm.companyId" placeholder="请选择公司" disabled>
  ...
  methods: {
    ...
    handleClose(done) {
      this.$confirm('确认关闭？')
          .then(_ => {
            this.editForm = {
              companyId: this.belongCompanyId
            };
            done();
          })
          .catch(_ => {
    ...
    getEmployList() {
      console.log(this.belongCompanyId)
    ...
    getBelongCompany() {
      this.$axios.post("/company/getBelongCompany", this.accountId).then(res => {
        ...
        this.editForm.companyId = res.data.data
        this.getEmployList()
      })
    }
  }
